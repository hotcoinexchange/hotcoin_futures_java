...
/**
 * WebSocket 用户订单推送订阅示例（需要登录鉴权）
 * WebSocket user order push subscription example (authentication required).
 *
 * <p>演示如何在完成 WebSocket 登录后，订阅当前账户的实时订单状态变动推送。
 * Demonstrates how to subscribe to real-time order status update push data
 * for the current account after WebSocket authentication.</p>
 *
 * <p>连接地址 / WebSocket URL: {@link HotcoinApiUrls#PRO_URL}</p>
 * <p>订阅类型 / Subscription type: {@code orders}</p>
...
public class OrderExample {
    ...
    /**
     * 构造用户订单订阅消息 JSON 字符串
     * Builds the JSON subscription message for the user orders channel.
     *
    ...
    static String paramsGenerate() {
        ...
        /** 订阅频道：用户订单 / Subscription channel: user orders */
    ...
    public static void main(String[] args) {
        /** 建立 WebSocket 连接并订阅用户订单（需要登录鉴权）
         *  Establish WebSocket and subscribe to user orders (authentication required) */
